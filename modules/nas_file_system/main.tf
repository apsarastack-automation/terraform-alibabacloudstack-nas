...
data "alibabacloudstack_nas_file_systems" "default" {
  ids = [var.nas_file_system_id]
}

# Create NAS file system (if not provided)
resource "alibabacloudstack_nas_file_system" "default" {
  count = length(data.alibabacloudstack_nas_file_systems.default.ids) > 0 ? 0 : 1
...
locals {
  file_system_id = length(data.alibabacloudstack_nas_file_systems.default.ids) > 0 ? var.nas_file_system_id : alibabacloudstack_nas_file_system.default[0].id
}
